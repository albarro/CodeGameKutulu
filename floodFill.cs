...
class Map
{
    ...
    public void addTravelCellCost(int parentCost, TaxiPoint center)
    {
        ...
        if(!MapArray[center.PosX, center.PosY].IsTravelCalculated){
            //Only add cost if parent cost bigger than 20
            if(parentCost> 20)
    ...
    public TaxiPoint getBestCell()
    {
        ...
        Cell bestCell = exploreMapAndGetBest(actualCell, actualCell, actualCell, 100);
    ...
    private Cell exploreMapAndGetBest(Cell actualBest, Cell previousCell, Cell actualCell, int maxDepth){
        //Console.Error.WriteLine("Actual Cell: " + actualCell);
        if(maxDepth <= 0){
            return actualBest;
        }

        if(actualCell.isWall()){
            return actualBest; //If wall we return
        }

        // We update travel costs and compare to best cell
        actualBest = compareAndUpdateCell(actualCell, previousCell.Cost, actualBest);
        //We have explore one level
        maxDepth -= 1;

        //We continue exploring if possible
        int x = actualCell.Cord.PosX;
        int y = actualCell.Cord.PosY;

        //Check left
        if(checkValidCell(x+1,y)){
            //Console.Error.WriteLine("Izq");
            actualBest = exploreMapAndGetBest(actualBest, actualCell, MapArray[x+1,y], maxDepth);
        }
        //Check right
        if(checkValidCell(x-1,y)){
            //Console.Error.WriteLine("Der");
            actualBest = exploreMapAndGetBest(actualBest, actualCell, MapArray[x-1,y], maxDepth);
        }

        //Check up
        if(checkValidCell(x,y-1)){
            //Console.Error.WriteLine("Arri");
            actualBest = exploreMapAndGetBest(actualBest, actualCell, MapArray[x,y-1], maxDepth);
        }
        //Check down
        if(checkValidCell(x,y+1)){
            //Console.Error.WriteLine("Abaj");
            actualBest = exploreMapAndGetBest(actualBest, actualCell, MapArray[x,y+1], maxDepth);
        }

        //Console.Error.WriteLine("Best Cell: " + actualBest);
    ...
    private bool checkValidCell(int x, int y){
        ...
        //Check if visited
        if(MapArray[x,y].IsTravelCalculated){
            return false;
        }
    ...
    private Cell compareAndUpdateCell(Cell cellToCompare, int travelCost, Cell bestCell){
        
        addTravelCellCost(travelCost, cellToCompare.Cord);

        return (cellToCompare.Cost < bestCell.Cost)? cellToCompare : bestCell;
    }
